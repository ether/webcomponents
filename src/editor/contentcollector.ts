...
export const sanitizeUnicode = (s: string): string => s.normalize('NFC');
const tagName = (n: Node): string | undefined => (n as HTMLElement).tagName && (n as HTMLElement).tagName.toLowerCase();
...
export const makeContentCollector = (collectStyles: boolean, abrowser: any, apool: AttributePool, className2Author: (cls: string) => string): ContentCollector => {
  const _blockElems: Record<string, number> = {
    div: 1,
    p: 1,
    pre: 1,
    li: 1,
  ...
  const textify = (str: string): string => sanitizeUnicode(
      str.replace(/(\n | \n)/g, ' ')
          .replace(/[\n\r ]/g, ' ')
          .replace(/\xa0/g, ' ')
          .replace(/\t/g, '        '));

  const getAssoc = (node: any, name: string): any => node[`_magicdom_${name}`];

  const lines = (() => {
    const textArray: string[] = [];
    const attribsArray: string[] = [];
    let attribsBuilder: SmartOpAssembler | null = null;
    const op = new Op('+');
    const self = {
      ...
      textOfLine: (i: number) => textArray[i],
      appendText: (txt: string, attrString: string = '') => {
  ...
  const cc = {} as ContentCollector;

  const _ensureColumnZero = (state: CCState | null) => {
    if (!lines.atColumnZero()) {
      cc.startNewLine(state);
    }
  };
  let selection: SelectionInfo | undefined;
  let startPoint: SelectionPoint | undefined;
  let endPoint: SelectionPoint | undefined;
  let selStart = [-1, -1];
  let selEnd = [-1, -1];
  const _isEmpty = (node: Node, state?: CCState | null) => {
  ...
  const _pointHere = (charsAfter: number, state: CCState) => {
  ...
  const _reachBlockPoint = (nd: Node, idx: number, state: CCState) => {
    if (nd.nodeType !== nd.TEXT_NODE) _reachPoint(nd, idx, state);
  };

  const _reachPoint = (nd: Node, idx: number, state: CCState) => {
  ...
  cc.incrementFlag = (state: CCState, flagName: string) => {
    state.flags[flagName] = (state.flags[flagName] || 0) + 1;
  };
  cc.decrementFlag = (state: CCState, flagName: string) => {
    state.flags[flagName]--;
  };
  cc.incrementAttrib = (state: CCState, attribName: string) => {
  ...
  cc.decrementAttrib = (state: CCState, attribName: string) => {
    state.attribs[attribName]--;
    _recalcAttribString(state);
  };

  const _enterList = (state: CCState, listType: string | undefined): string | undefined => {
    if (!listType) return;
    const oldListType = state.lineAttributes.list as string | undefined;
    if (listType !== 'none') {
  ...
  const _exitList = (state: CCState, oldListType: string) => {
  ...
  const _enterAuthor = (state: CCState, author: string): string | undefined => {
  ...
  const _exitAuthor = (state: CCState, oldAuthor: string | undefined) => {
    state.authorLevel = (state.authorLevel || 0) - 1;
    state.author = oldAuthor;
    _recalcAttribString(state);
  };

  const _recalcAttribString = (state: CCState) => {
  ...
  const _produceLineAttributesMarker = (state: CCState) => {
  ...
  cc.startNewLine = (state: CCState | null) => {
    if (state) {
  ...
  cc.notifySelection = (sel: SelectionInfo | null) => {
    if (sel) {
      selection = sel;
  ...
  cc.doAttrib = (state: CCState, na: string) => {
    state.localAttribs = (state.localAttribs || []);
    state.localAttribs.push(na);
    cc.incrementAttrib(state, na);
  };
  cc.collectContent = function (this: ContentCollector, node: Node, state?: CCState) {
    let unsupportedElements: Set<string> | null = null;
    if (!state) {
      state = {
    ...
    if (!isBlock && (node as any).name && ((node as any).name !== 'body')) {
      if (!supportedElems.has((node as any).name)) state.unsupportedElements.add((node as any).name);
    }
    ...
    if (node.nodeType === node.TEXT_NODE) {
      const tname = (node.parentNode as HTMLElement).getAttribute('name');
      ...
      if (tname === 'br') {
        ...
        let isPre: boolean | RegExpExecArray | null = (tname === 'pre');
        if ((!isPre) && abrowser && abrowser.safari) {
          isPre = styl ? /\bwhite-space:\s*pre\b/i.exec(styl) : null;
        }
        if (isPre) cc.incrementFlag(state, 'preMode');
        let oldListTypeOrNull: string | null = null;
        let oldAuthorOrNull: string | null = null;
        ...
        if (collectStyles) {
          const preContext = {cc: cc as any, state: state as any, tname, styl, cls: cls || ''};
          ...
          if (tname === 'ul' || tname === 'ol') {
            let type: string | null = el.getAttribute('class');
            ...
            if (!rr && !type) {
              for (let ci = 0; ci < node.childNodes.length; ci++) {
                ...
                type = (child as HTMLElement).getAttribute('class');
                if (type) break;
              }
            }
            if (rr && rr[1]) {
        ...
        if (collectStyles) {
          const postContext = {cc: cc as any, state: state as any, tname, styl, cls};
  ...
  cc.notifyNextNode = (node: Node | null) => {
