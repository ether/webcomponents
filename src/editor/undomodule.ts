...
interface ExternalChange {
  elementType: 'externalChange';
  changeset: string;
}

type StackElement = UndoableEvent | ExternalChange;
...
export interface UndoModule {
  clearHistory: () => void;
  reportEvent: (event: ReportedEvent) => void;
  reportExternalChange: (changeset: string) => void;
  performUndo: (eventFunc: (changeset?: string, selInfo?: SelectionInfo | null) => any) => void;
  performRedo: (eventFunc: (changeset?: string, selInfo?: SelectionInfo | null) => any) => void;
  enabled: boolean;
  apool: AttributePool | null;
}

export const undoModule: UndoModule = (() => {
  const stack = (() => {
    ...
    const UNDOABLE_EVENT = 'undoableEvent' as const;
    const EXTERNAL_CHANGE = 'externalChange' as const;
    ...
    const pushEvent = (event: ReportedEvent) => {
      const e: UndoableEvent = {
        ...event,
        elementType: UNDOABLE_EVENT,
    ...
    const pushExternalChange = (cs: string) => {
      const idx = stackElements.length - 1;
      if (stackElements[idx].elementType === EXTERNAL_CHANGE) {
        const ext = stackElements[idx] as ExternalChange;
        ext.changeset = compose(ext.changeset, cs, getAPool());
      } else {
        stackElements.push(
            {
    ...
    const _exposeEvent = (nthFromTop: number) => {
      ...
      while (idx > targetIndex || stackElements[idx].elementType === EXTERNAL_CHANGE) {
        if (stackElements[idx].elementType === EXTERNAL_CHANGE) {
          const ex = stackElements[idx] as ExternalChange;
          const un = stackElements[idx - 1] as UndoableEvent;
          if (un.backset) {
            ...
            if ((typeof un.selStart) === 'number') {
              const newSel = characterRangeFollow(excs, un.selStart!, un.selEnd!, 0);
          ...
          if (idx >= 2 && stackElements[idx - 2].elementType === EXTERNAL_CHANGE) {
            ex.changeset =
                compose((stackElements[idx - 2] as ExternalChange).changeset, ex.changeset, getAPool());
    ...
    const getNthFromTop = (n: number): UndoableEvent => {
      // precond: 0 <= n < numEvents()
      _exposeEvent(n);
      return stackElements[stackElements.length - 1 - n] as UndoableEvent;
    };

    const numEvents = () => numUndoableEvents;

    const popEvent = (): UndoableEvent => {
      // precond: numEvents() > 0
      _exposeEvent(0);
      numUndoableEvents--;
      return stackElements.pop() as UndoableEvent;
    };
  ...
  const _charOccurrences = (str: string, c: string) => {
    let i = 0;
    let count = 0;
  ...
  const _opcodeOccurrences = (cs: string, opcode: string) => _charOccurrences(unpack(cs).ops, opcode);

  const _mergeChangesets = (cs1: string, cs2: string | undefined): string | null => {
    if (!cs1) return cs2 || null;
    if (!cs2) return cs1;
  ...
  const reportExternalChange = (changeset: string) => {
    if (changeset && !isIdentity(changeset)) {
      stack.pushExternalChange(changeset);
    }
  };

  const _getSelectionInfo = (event: UndoableEvent): SelectionInfo | null => {
    if ((typeof event.selStart) !== 'number') {
      return null;
    } else {
      return {
  ...
  const performUndo = (eventFunc: (changeset?: string, selInfo?: SelectionInfo | null) => any) => {
  ...
  const performRedo = (eventFunc: (changeset?: string, selInfo?: SelectionInfo | null) => any) => {
    if (undoPtr >= 2) {
  ...
  const getAPool = (): AttributePool => undoModule.apool!;
